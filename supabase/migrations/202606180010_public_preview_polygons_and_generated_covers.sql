-- Public map previews preserve the owner-drawn shape without exposing the
-- exact private boundary. The preview is simplified, snapped, and translated
-- to the rounded public point that is already safe to show.

create or replace function public.make_public_boundary_preview(
  p_boundary geometry,
  p_exact_point geography,
  p_public_point geography
)
returns jsonb
language sql
immutable
...
grant execute on function public.make_public_boundary_preview(geometry, geography, geography) to anon, authenticated, service_role;
