-- Owners may provide an acreage from a deed, survey, or land record while the
-- drawn polygon remains the source for map UX, geospatial search, and privacy.
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'listings_reported_area_positive'
  ) then
    alter table public.listings
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'listings_area_display_source_check'
  ) then
    alter table public.listings
